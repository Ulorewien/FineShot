...
def inference(image_path, annotation_path, dataset="refcoco"):
    ...
    for image_path in tqdm(image_paths, desc="Inference using YOLO-CLIP-VLM"):
        try:
            if dataset == "refcoco":
                detections = yolo_clip_vlm(image_path, annotations[str(i)]["labels"], confidence_threshold=0.5)
            else:
                detections = yolo_clip_vlm(image_path, annotations[i]["labels"], confidence_threshold=0.5)
...
if __name__ == "__main__":
    ...
    with open("/data/yashowardhan/FineShot/test/results/yolo_clip_vlm_refcoco_results.json", "w") as f:
        json.dump(refcoco_results, f, indent=4)
    
    vg_results = inference(vg_image_path, vg_annotation_path, "vg")
    with open("/data/yashowardhan/FineShot/test/results/yolo_clip_vlm_vg_results.json", "w") as f:
